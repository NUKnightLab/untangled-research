...
def catch_all(path='index.html'):
    ...
    if path.endswith('.md'):
        markdown_path = os.path.join(os.curdir,'website/templates/',path)
        markdown_content = open(markdown_path).read()
        path = "base.html"
    return render_template(path,markdown_content=markdown_content)
